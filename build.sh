...
build_aur_package() {
  ...
  find "$package_dir" -maxdepth 1 -type f -name '*.pkg.tar.*' -exec cp -f {} "$LOCAL_REPO_DIR/" \;
}
...
main() {
  check_arch_host
  check_dependencies
  [[ ${EUID} -ne 0 ]] || fail "Run build.sh as a regular user. The script will call sudo only for mkarchiso."
